...
func (r *WordSetRepository) List(ctx context.Context, userID uuid.UUID) ([]domain.WordSet, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT id, user_id, name, icon, mode, is_default, auto_generate_new_words, new_word_source, recording_enabled, enabled_review_modes, created_at, updated_at
		FROM word_sets
		WHERE user_id = $1
...
func (r *WordSetRepository) Create(ctx context.Context, set domain.WordSet) (domain.WordSet, error) {
	...
	return scanWordSet(r.pool.QueryRow(ctx, `
		INSERT INTO word_sets (user_id, name, icon, mode, is_default, auto_generate_new_words, new_word_source, recording_enabled, enabled_review_modes)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		RETURNING id, user_id, name, icon, mode, is_default, auto_generate_new_words, new_word_source, recording_enabled, enabled_review_modes, created_at, updated_at
	`, set.UserID, name, set.Icon, string(mode), set.IsDefault, set.AutoGenerateNewWords, newWordSourceValue(set.NewWordSource), set.RecordingEnabled, reviewModeValues(set.EnabledReviewModes)))
}
...
func (r *WordSetRepository) Update(ctx context.Context, set domain.WordSet) (domain.WordSet, error) {
	...
	return scanWordSet(r.pool.QueryRow(ctx, `
		...
		SET name = $3, icon = $4, mode = $5,
		    auto_generate_new_words = $6, new_word_source = $7, recording_enabled = $8, enabled_review_modes = $9
		WHERE user_id = $1 AND id = $2
		RETURNING id, user_id, name, icon, mode, is_default, auto_generate_new_words, new_word_source, recording_enabled, enabled_review_modes, created_at, updated_at
	`, set.UserID, set.ID, name, set.Icon, string(mode), set.AutoGenerateNewWords, newWordSourceValue(set.NewWordSource), set.RecordingEnabled, reviewModeValues(set.EnabledReviewModes)))
}
...
func (r *WordSetRepository) EnsureDefault(ctx context.Context, userID uuid.UUID) (domain.WordSet, error) {
	...
	return scanWordSet(r.pool.QueryRow(ctx, `
		INSERT INTO word_sets (user_id, name, icon, mode, is_default, auto_generate_new_words, new_word_source)
		VALUES ($1, 'Default', 'default', 'new_words', TRUE, TRUE, 'llm')
...
func newWordSourceValue(source domain.NewWordSource) string {
	if source == "" {
		return string(domain.NewWordSourceLLM)
	}
