...
func (r *LearningEventRepository) Insert(ctx context.Context, event domain.LearningEvent) error {
	query := `
		INSERT INTO learning_events (
			user_id, word_id, pool_item_id, event_type, event_time, payload, response_time_ms, mode_used, client_event_id
		) VALUES (
...
func (r *LearningEventRepository) ListRecentByPoolItem(ctx context.Context, itemID uuid.UUID) ([]domain.LearningEvent, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT id, user_id, word_id, pool_item_id, event_type, event_time, payload, response_time_ms, mode_used, client_event_id, created_at
