...
func TestRouterWithDevAuthSettingsAndPool(t *testing.T) {
	...
	router := NewRouter(config.Config{AdminToken: "secret"}, logger, nil, verifier, identity, settingsService, dictionaryService, poolService, learningService, dynamicReviewService, nil, nil, statisticsService, nil, llmRepo, nil, BuildInfo{})
...
func TestTestLLMEndpoint(t *testing.T) {
	...
	statisticsService := service.NewStatisticsService(settingsRepo, wordRepo, stateRepo, eventRepo, clock)
	router := NewRouter(config.Config{AdminToken: "secret"}, logger, nil, verifier, identity, settingsService, dictionaryService, poolService, learningService, dynamicReviewService, nil, nil, statisticsService, nil, llmRepo, &staticPromptTester{}, BuildInfo{})
